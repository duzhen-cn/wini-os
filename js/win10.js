...
window.Win10 = {
    ...
    _renderContextMenu: function(x, y, menu, trigger) {
        $("#frame")[0].contentWindow.removeContextMenu();
    ...
    _init: function() {
        ...
        $("#win10_btn_refresh").click(function() {
            loadData();
        });
        $("#win10_btn_command").click(function() {
            $("#win10-desktop-scene").addClass("bg-blur");
            Win10.menuClose();
            Win10.commandCenterToggle();
        });
        $("#win10-desktop-scene").click(function() {
            Win10.commandCenterToggle();
        });
        ...
        //打广告
        setTimeout(function() {
            console.log(Win10.lang('Welcome to Wini OS developer mode.'));
            console.log("Author：Thomas Wechat:wenxuejn");
            console.log("Based on the WIN10-UI and created by Yuri2")
        }, 2000);
        //点击清空右键菜单
        ...
        Win10.setContextMenu('#win10>.desktop', [
            ['<i class="fa fa-fw fa-refresh"></i> ' + Win10.lang('Refresh List', 'Rereshs'), function() { loadData(); }],

            ['<i class="fa fa-fw fa-window-maximize"></i> ' + Win10.lang('Full screen', 'Enable Full Screen'), function() { Win10.enableFullScreen() }],
            ['<i class="fa fa-fw fa-window-restore"></i> ' + Win10.lang('Disable Full', 'Disable Full Screen'), function() { Win10.disableFullScreen() }],
            '|', ['<i class="fa fa-fw fa-info-circle"></i> ' + Win10.lang('About Us', 'About Us'), function() { Win10.aboutUs() }],
        ]);
    ...
    menuToggle: function() {
        if (!$("#win10-menu").hasClass('opened')) {
            //$("#win10-menu").addClass("bg-blur");
            this.menuOpen();
        } else {
            //$("#win10-menu").removeClass("bg-blur");
            this.menuClose();
        }
    },
    addGosBlur: function() {
    ...
    reemoveGosBlur: function() {
        $("#win10-desktop-scene").removeClass("bg-blur");
    },
    ...
    renderShortcuts: function() {
        // var h=parseInt($("#win10 #win10-shortcuts")[0].offsetHeight/100);//注释防止js报错，如需改为win10桌面布局，取消注释即可
        var h = 0;
        var x = 0,
            y = 0;
    ...
    fallScreen: function() {
        ...
        if (explorer.indexOf("Chrome") < 0) {
            // alert("未检测到监控插件,稍后将进行安装！");
            $("#win10-desktop-scene").removeClass("bg-blur");
            layer.msg('Browser detection failed, please download the Google browser.', {
                time: 1000000000, //20s后自动关闭
                shade: 0.8,
                btn: ['Now', 'Letter'],
                yes: function(index) {

                    window.open("./component/ChromeStup.exe", "_self");

                },
                btn2: function(index) {
                    $("#win10-desktop-scene").addClass("bg-blur");
                    var index = layer.msg('Does the first load enter full screen?', {
                        time: 100000000, //20s后自动关闭
                        //area: ['200px', '30px'],
                        shade: 0.1,
                        btn: ['Now', 'Letter'],
                        yes: function(index) {
    ...
    exit: function() {
        $("#win10-menu").removeClass('opened');
        $("#win10-menu").addClass('hidden');
        $("#win10").addClass("bg-blur");
        layer.confirm(Win10.lang('Are you sure you want to exit the system?', 'Are you sure you want to close this page?'), { icon: 3, title: Win10.lang('Prompt', 'Prompt'), btn: ['Ok', 'Cancle'], closeBtn: 0, anim: 5 }, function(index) {
    ...
    aboutUs: function() {
        ...
        layer.open({
            type: 1,
            closeBtn: 1, //不显示关闭按钮
            // skin: ' body .demo-class .layui-layer-btn .layui-layer-btn1{background:#999;}',
            anim: 2,
...
/*
 * @stationNum 为此次进入回环的机房数量的奇偶性 （1为奇数，2为偶数）
 * @nameA 为上半部机房名称
 * @nameB 为下半部机房名称
 * @stationIdA 为上半部机房ID
 * @stationIdB 下半部将ID
...
//首页机房状态回环显示
function fresh() {
    var time = 4000; //控制刷新时间 1000为1秒
    var data = [{ "areaname": "Station One", "id": 1 }, { "areaname": "Station Two", "id": 2 }, { "areaname": "Station Three", "id": 3 }, { "areaname": "Station Four", "id": 3 }, { "areaname": "Station Five", "id": 3 }];
    var stationArr = new Array(); //机房名称列表，也可不用
    //机房数量大于3时，进入完全回环模式
    if (data.length > 3) {
        $("#leftFresh").show();
        for (i = 0; i < data.length; i++) {
            stationArr.push(data[i].id);
        }
        var numA = 2; //设置上半部从第3个机房开始，0为第一个
        var numB = 3; //设置下半部从第4个机房开始，0为第一个
        $("#frame").attr("src", 'right.html?stationNum=2&nameA=' + data[0].areaname + '&nameB=' + data[1].areaname + '&stationIdA=' + data[0].id + '&stationIdB=' + data[1].id);
        setTimeout(function() {
            changeURL("stationNum=2&nameA=" + data[numA].areaname + "&nameB=" + data[numB].areaname + "&stationIdA=" + data[numA].id + "&stationIdB=" + data[numB].id);
            numA += 2;
            numB += 2;
            if (numB <= stationArr.length - 1) {
                setTimeout(arguments.callee, time); //调用函数
            } else {
                if (numA <= stationArr.length - 1) {
                    setTimeout(function() {
                        $("#frame").attr("src", 'right.html?stationNum=1&nameA=' + data[numA].areaname + '&nameB=&stationIdA=' + data[numA].id + '&stationIdB=');
                    }, time);
                    setTimeout(function() {
                        fresh();
                    }, time * 2); // time * 2 等待上一次刷新完成
                } else {
                    setTimeout(function() {
                        fresh();
                    }, time); // time * 2 等待上一次刷新完

                }
            }
        }, time);

    } //机房数量等于3时，进入不完全回环模式，防止数组溢出	
    else if (data.length == 3) {
        $("#leftFresh").show();
        $("#frame").attr("src", 'right.html?stationNum=2&nameA=' + data[0].areaname + '&nameB=' + data[1].areaname + '&stationIdA=' + data[0].id + '&stationIdB=' + data[1].id);
        setTimeout(function() {
            changeURL("stationNum=1&nameA=" + data[2].areaname + "&nameB=&stationIdA=" + data[2].id + "&stationIdB=");
        }, time);
        console.log('10s后开始回环');
        setTimeout(function() {
            fresh();
        }, time * 2); // time * 2 等待上一次刷新完成	
        ...
        $("#leftFresh").show();
        $("#frame").attr("src", 'right.html?stationNum=1&nameA=' + data[0].areaname + '&nameB=&stationIdA=' + data[0].id + '&stationIdB=');
        setInterval(function() {
            $("#frame").attr("src", 'right.html?stationNum=1&nameA=' + data[0].areaname + '&nameB=&stationIdA=' + data[0].id + '&stationIdB=');
        ...
        $("#leftFresh").hide();
        layer.msg('<h1><img src="img/mark/AlarmTip.png">   暂无数据</h1>', {
            time: time - 500 //关闭（如果不配置，默认是3秒）
        });

        setTimeout(function() {
            loadDataNoMsg(); //重新请求树形菜单数据
            fresh(); //重新请求机房数据
        }, time);
    }
}
...
function freshfaile() {
    $("#frame").hide();
    $("#win10-menu").addClass('hidden');
    layer.msg('<h1><img src="img/mark/NoNet.png">  Connection failed.</h1>', {
        time: 10000 //关闭（如果不配置，默认是3秒）
    });
    setTimeout(function() {
        $("#frame").show();
        loadDataNoMsg(); //重新请求树形菜单数据
...
function changeURL(data) {
    $("#frame").attr("src", 'right.html?' + data);
}